...
void DSMKeeper::initLocalMeta() {
  ...
  localMeta.cacheBase = (uint64_t)cacheCon[0]->cachePool;
  ...
  for (int i = 0; i < NR_CACHE_AGENT; ++i) {
    localMeta.cacheTh[i].lid = cacheCon[i]->ctx.lid;
    localMeta.cacheTh[i].rKey = cacheCon[i]->cacheMR->rkey;
    memcpy((char *)localMeta.cacheTh[i].gid, (char *)(&cacheCon[i]->ctx.gid),
           16 * sizeof(uint8_t));

    localMeta.cacheUdQpn[i] = cacheCon[i]->message->getQPN();
  }
}
...
void DSMKeeper::setDataToRemote(uint16_t remoteID) {
  for (int i = 0; i < NR_DIRECTORY; ++i) {
    ...
    for (int k = 0; k < NR_CACHE_AGENT; ++k) {
      localMeta.dirRcQpn2cache[i][k] = c->data2cache[k][remoteID]->qp_num;
    }
  ...
  for (int i = 0; i < NR_CACHE_AGENT; ++i) {
    ...
    for (int k = 0; k < NR_DIRECTORY; ++k) {
      localMeta.cacheRcQpn2dir[i][k] = c->data[k][remoteID]->qp_num;
    }

    for (int k = 0; k < MAX_APP_THREAD; ++k) {
      localMeta.cacheRcQpn2app[i][k] = c->toApp[k][remoteID]->qp_num;
    }
  }
  ...
  for (int i = 0; i < MAX_APP_THREAD; ++i) {
    ...
    for (int k = 0; k < NR_CACHE_AGENT; ++k) {
      localMeta.appRcQpn2cache[i][k] = c->fromAgent[k][remoteID]->qp_num;
    }
  }
}
...
void DSMKeeper::setDataFromRemote(uint16_t remoteID, ExchangeMeta *remoteMeta) {
  for (int i = 0; i < NR_DIRECTORY; ++i) {
    auto &c = dirCon[i];
    for (int k = 0; k < NR_CACHE_AGENT; ++k) {
      auto &qp = c->data2cache[k][remoteID];

      assert(qp->qp_type == IBV_QPT_RC);
      modifyQPtoInit(qp, &c->ctx);
      modifyQPtoRTR(qp, remoteMeta->cacheRcQpn2dir[k][i],
                    remoteMeta->cacheTh[k].lid, remoteMeta->cacheTh[k].gid,
                    &c->ctx);
      modifyQPtoRTS(qp);
    }
  ...
  for (int i = 0; i < NR_CACHE_AGENT; ++i) {
    ...
    for (int k = 0; k < NR_DIRECTORY; ++k) {
      ...
      assert(qp->qp_type == IBV_QPT_RC);
      modifyQPtoInit(qp, &c->ctx);
      modifyQPtoRTR(qp, remoteMeta->dirRcQpn2cache[k][i],
    ...
    for (int k = 0; k < MAX_APP_THREAD; ++k) {
      auto &qp = c->toApp[k][remoteID];

      assert(qp->qp_type == IBV_QPT_RC);
      modifyQPtoInit(qp, &c->ctx);
      modifyQPtoRTR(qp, remoteMeta->appRcQpn2cache[k][i],
                    remoteMeta->appTh[k].lid, remoteMeta->appTh[k].gid,
                    &c->ctx);
      modifyQPtoRTS(qp);
    }
  ...
  for (int i = 0; i < MAX_APP_THREAD; ++i) {
    ...
    for (int k = 0; k < NR_CACHE_AGENT; ++k) {
      auto &qp = c->fromAgent[k][remoteID];

      assert(qp->qp_type == IBV_QPT_RC);
      modifyQPtoInit(qp, &c->ctx);
      modifyQPtoRTR(qp, remoteMeta->cacheRcQpn2app[k][i],
                    remoteMeta->cacheTh[k].lid, remoteMeta->cacheTh[k].gid,
                    &c->ctx);
      // modifyQPtoRTS(qp);
    }
  ...
  for (int i = 0; i < NR_DIRECTORY; ++i) {
    ...
    for (int k = 0; k < NR_CACHE_AGENT; ++k) {
      ...
      fillAhAttr(&ahAttr, remoteMeta->dirTh[i].lid, remoteMeta->dirTh[i].gid,
                 &cacheCon[k]->ctx);
      info.cacheToDirAh[k][i] = ibv_create_ah(cacheCon[k]->ctx.pd, &ahAttr);

      assert(info.cacheToDirAh[k][i]);
    }
  ...
  for (int i = 0; i < NR_CACHE_AGENT; ++i) {
    info.cacheRKey[i] = remoteMeta->cacheTh[i].rKey;
    info.cacheMessageQPN[i] = remoteMeta->cacheUdQpn[i];

    for (int k = 0; k < NR_DIRECTORY; ++k) {
      struct ibv_ah_attr ahAttr;
      fillAhAttr(&ahAttr, remoteMeta->cacheTh[i].lid,
                 remoteMeta->cacheTh[i].gid, &dirCon[k]->ctx);
      info.dirToCacheAh[k][i] = ibv_create_ah(dirCon[k]->ctx.pd, &ahAttr);
      assert(info.dirToCacheAh[k][i]);
    }
  }
...
void DSMKeeper::initRouteRule() {
  ...
  barrier("keeper-route-start");

  if (this->getMyNodeID() == 0) {
    // for (int i = 0; i < this->getServerNR(); ++i) {
    // std::string ki = std::string(ServerPrefix) + std::to_string(i);
    // char *ip = memGet(ki.c_str(), ki.size());
    // Debug::notifyInfo("%s", ip);
    // serverList.push_back(std::string(ip));
    // free(ip);
    // }
    system("ssh -i /home/tzr/.ssh/id_ed25519 tzr@192.168.189.34 'sudo /home/tzr/nfs/concordia/p4src/table.py'");
    system("ssh -i /home/tzr/.ssh/id_ed25519 tzr@192.168.189.34 'sudo /home/tzr/nfs/concordia/p4src/mc.py'");
  }
