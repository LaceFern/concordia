...
action after_unlock_act() {
    /* loop_back(); */
    /* modify_field(message.mtype, ACK_UNLOCK); */
    drop();
}
...
table after_unlock_tbl {
    ...
    default_action: after_unlock_act;
}
...
#define LOCK_W_LOCK_ALU(i)                \
blackbox stateful_alu lock_w_alu_##i {    \
    reg: lock_##i;                        \
                                          \
    condition_lo : register_lo == UNLOCK; \
                                          \
    update_lo_1_predicate: condition_lo;  \
    update_lo_1_value : W_LOCK;           \
                                          \
...
/*
    reg: lock_##i;                            \
                                              \
    condition_lo : register_lo != W_LOCK;     \
                                              \
    update_lo_1_predicate: condition_lo;      \
...
#define LOCK_R_LOCK_ALU(i)                    \
blackbox stateful_alu lock_r_alu_##i {        \
   reg: lock_##i;                        \
                                          \
    condition_lo : register_lo == UNLOCK; \
                                          \
...
#define LOCK_W_LOCK_ACT(i) \
action w_lock_tbl_act_##i() { \
    lock_w_alu_##i.execute_stateful_alu(message.index); \
}

#define LOCK_R_LOCK_ACT(i) \
action r_lock_tbl_act_##i() { \
    lock_r_alu_##i.execute_stateful_alu(message.index); \
}
