...
requester_arp_script = "arp-r2-3.sh"

home_machine = "192.168.189.9"
home_nic_name = "enp28s0np0"
home_arp_script = "arp-r2-3.sh"
home_node_id = 2
...
def master_run(ssh, program, app_thread_num, output_dir, node_num, nic_name):
...
def requester_run(ssh, program, app_thread_num, output_dir, node_num, nic_name):
    print("export NIC_NAME={7} && cd {0}/build && sudo -E ./{1} "
        "--no_node {2} --no_thread {3} "
        "--locality 0 --shared_ratio 100 --read_ratio 50 "
        "--is_cache 0 --cache_rw 0 --is_request 1 --request_rw {6} --is_home 0 --home_node_id {4} "
        "--result_dir {5}".format(base, program, node_num, app_thread_num, home_node_id, output_dir, request_type, nic_name))
...
def home_run(ssh, program, app_thread_num, output_dir, node_num, nic_name):
    print("export NIC_NAME={6} && cd {0}/build && sudo -E ./{1} "
        "--no_node {2} --no_thread {3} "
        "--locality 0 --shared_ratio 100 --read_ratio 50 "
...
def cache_run(ssh, program, app_thread_num, output_dir, node_num, nic_name):
    print("export NIC_NAME={7} && cd {0}/build && sudo -E ./{1} "
        "--no_node {2} --no_thread {3} "
        "--locality 0 --shared_ratio 100 --read_ratio 50 "
