...
4. 若在运行sudo -E ./auto_run.sh时交换机出现下图左1 resource busy报错（左2和左3是因交换机出错导致服务器运行报错），说明交换机上已经有一个程序占着在跑了，通常是hello_bfrt，可以用sudo killall hello_bfrt杀掉（如果没用请联系管理员）
![resource busy error](images/resource_busy.png)
...
设置网卡mtu为4200：sudo ifconfig [NIC name] mtu 4200 (注：已经写在arp-xxx.sh中了)

在build目录下，加载arp条目：sudo bash ../arp-[node id].sh

# 设置服务器大页（每台）
在build目录下，设置大页：sudo bash ../hugepage.sh
